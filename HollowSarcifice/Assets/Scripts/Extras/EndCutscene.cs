...
public class EndCutscene : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0) && (!inNPCZone || allowSkip))
        {
            ...
            // If typing is finished and we're on the final line → END CUTSCENE
    ...
    public void StartDialogue()
    {
        ...
        typingRoutine = StartCoroutine(TypeLine());   // <-- track the routine
    }

    IEnumerator TypeLine()
    {
        finishedTyping = false;  // <-- NEW
    ...
    void NextLine()
    {
        if (index < lines.Length - 1)
        {
            ...
            // Change image if index is in imageSwap[]
